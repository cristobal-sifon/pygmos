  ...
  for line in log:
    ...
    if line[:17] == 'GSWAVELENGTH exit':
      try:
        print 'Mask', mask
        print image
        print 'Wavelength calibration RMS (values over all slits, in Angstrom):'
        print '  median=%.3f    min=%.3f    max=%.3f' %(numpy.median(slits), min(slits), max(slits))
        print '  median_std=%.3f   min_std=%.3f    max_std=%.3f' %(numpy.median(sdev), min(sdev), max(sdev))
        print '-' * 50
        print '-' * 50
      except ValueError:
        pass
