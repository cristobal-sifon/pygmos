...
def mos(args, cluster, mask, scienceFiles, assoc, cutdir,
    ...
    if args.ns:
        ...
        for science in scienceFiles.keys():
            ...
            if not arc:
                print('Not enough data for mask {2} (science file' \
                      ' {1})'.format(mask, science))
                continue
            ...
            dark = tasks.Call_gbias(
                darks, fl_over='no', fl_trim='yes', fl_vardq='no',
                fl_inter='no', median='no')
            science = tasks.Call_gprepare(
                science, fl_vardq='no', fl_addmdf='yes')
            science = tasks.Call_gireduce(
                science, bias=dark, fl_over='no', fl_trim='yes',
                fl_bias='yes', fl_dark='no', fl_flat='no', fl_addmdf='no')
    ...
    # regular observations
        ...
        for science in scienceFiles.keys():
            ...
            if not flat:
                print('Not enough data for mask {0} (science file' \
                      ' {1})'.format(mask, science))
                continue
...
def longslit(args, cluster, waves, assoc, ns=False):
    """
